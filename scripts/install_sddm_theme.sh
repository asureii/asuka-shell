...
echo "=== Installing Evangelion Unit-02 (Asuka) SDDM Theme ==="
...
if [ "$EUID" -ne 0 ]; then
    ...
    echo -e "[Theme]\nCurrent=evangelion-asuka" | sudo tee /etc/sddm.conf.d/theme.conf > /dev/null
else
    mkdir -p "$THEME_DEST"
    cp -r "$THEME_SRC"/* "$THEME_DEST"/
    chmod -R a+rX "$THEME_DEST"
    mkdir -p /etc/sddm.conf.d/
    echo -e "[Theme]\nCurrent=evangelion-asuka" > /etc/sddm.conf.d/theme.conf
fi

echo "✓ Evangelion SDDM Theme installed and set as active theme in /etc/sddm.conf.d/theme.conf!"
